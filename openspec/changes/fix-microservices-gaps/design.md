...
- No change to the `AuditEvent` schema, the `AuditConsumer`, or the `AuditLog` persistence — the consumer side already works and is specified in `async-event-consumers`.
