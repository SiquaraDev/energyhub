...
Consul reflita a saúde de cada instância e os chamadores (e o Traefik) a resolvam por nome. Como o
`Name` continua sendo o **nome lógico** do serviço, a descoberta por nome e o roteamento
Consul-catalog do Traefik não mudam; o que muda é que **cada réplica passa a ter uma entrada própria
no catálogo**, em vez de sobrescreverem umas às outras.
...
def register_with_consul(
    *,
    ...
    O `instance_id` é o HOSTNAME do pod (k8s) ou um uuid4 por processo (Compose/local), tornando o id
    **único por réplica** — cada uma tem a sua própria entrada no catálogo. O `Name` continua sendo o
    **nome lógico** do serviço, de modo que a descoberta por nome e o roteamento Consul-catalog do
    Traefik permanecem inalterados.
