"""Ponto de entrada do **contract-service** (Fase 15).

Microsserviço do contexto **Contracts**. Upstream: Auth (via `AuthClient`, para o usuário atual) e
Clients (via `ClientClient`, para dados de cliente). Dono do seu banco (`contractdb`).
"""
...
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    ...
    _service_id = register_with_consul(
        ...
        tags=[
            "traefik.enable=true",
            "traefik.http.routers.contracts.rule=PathPrefix(`/api/v1/contracts`)",
            "traefik.http.routers.contracts.entrypoints=web",
            "traefik.http.routers.contracts.middlewares=eh-ratelimit,eh-auth",
        ],
    )
...
app = FastAPI(
    title="EnergyHub — Contract Service",
    version=settings.app_version,
    lifespan=lifespan,
    description="Microsserviço de contratos de energia.",
)
