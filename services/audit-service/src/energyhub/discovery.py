...
(`{name}-{instance_id}`, onde o `instance_id` é o HOSTNAME do pod ou um uuid4 por processo),
...
própria no catálogo, em vez de sobrescreverem umas às outras sob um id compartilhado.
...
def register_with_consul(
    *,
    ...
    O `instance_id` é o HOSTNAME do pod (em k8s) ou um uuid4 por processo, o que dá a cada réplica
    uma entrada própria no catálogo. O `Name` registrado continua sendo o **nome lógico** do serviço,
    então a descoberta por nome e o roteamento Consul-catalog do Traefik não mudam.
